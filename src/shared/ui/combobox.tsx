...
function Combobox({
  ...
  Omit<
    React.ComponentPropsWithoutRef<typeof ComboboxRoot>,
    'value' | 'onValueChange'
  >) {
  return (
    <ComboboxRoot
...
>) {
  ...
  return (
    <ComboboxRoot
      value={value}
      onValueChange={(val: any) => onValueChange?.(val as Array<string>)}
      disabled={disabled}
      multiple
      {...props}
    >
      ...
      >
        <ComboboxPrimitive.Value>
          {(values: any) =>
            (values as Array<string>).map((val) => {
              const option = options.find((opt) => opt.value === val)
              return (
                <ComboboxChip key={val}>{option?.label ?? val}</ComboboxChip>
              )
            })
          }
        </ComboboxPrimitive.Value>
